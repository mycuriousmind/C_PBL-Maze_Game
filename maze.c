...
static void remove_extra_walls(int rows, int cols, int removal_percent) {
     ...
     */
    typedef struct { int r; int c; } WallPos;
    ...
    for (r = 1; r < rows - 1; r++) {
        for (c = 1; c < cols - 1; c++) {
            if (maze[r][c] != TILE_WALL) continue;
...
void draw_maze(int player_row, int player_col,
    ...
    if (exit_row >= 0 && exit_col >= 0) {
        ...
        int pad = cell_size / 8;
        DrawRectangle(x + pad, y + pad, cell_size - 2 * pad, cell_size - 2 * pad, GOLD);
    ...
    if (bot_row >= 0 && bot_col >= 0) {
        ...
        int pad = cell_size / 8;
        DrawRectangle(x + pad, y + pad, cell_size - 2 * pad, cell_size - 2 * pad, RED);
        int font = cell_size / 3;
        if (font < 8) font = 8;
    ...
    if (player_row >= 0 && player_col >= 0) {
        ...
        int pad = cell_size / 8;
        DrawRectangle(x + pad, y + pad, cell_size - 2 * pad, cell_size - 2 * pad, BLUE);
        int font = cell_size / 3;
        if (font < 8) font = 8;
