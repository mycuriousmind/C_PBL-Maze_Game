...
static double botMoveTimer = 0.0;
static double botMoveInterval = 0.35;  /* seconds */
...
static void init_level(int level) {
  ...
  if (level == 0)
    botMoveInterval = 0.60;
  else if (level == 1)
    botMoveInterval = 0.40;
  else
    botMoveInterval = 0.25;
...
static void move_bot(void) {
  ...
  if (astar_find_path(bot_row, bot_col, player_row, player_col, &next_row,
                      &next_col)) {
    bot_row = next_row;
    bot_col = next_col;
  }
...
static void draw_hud(void) {
  ...
  /* Level and Turn */
  snprintf(buf, sizeof(buf), "Level: %d/%d   Turn: %d", currentLevel + 1,
...
static void draw_transition_screen(void) {
  ...
  if (currentLevel + 1 < NUM_LEVELS) {
    ...
    snprintf(buf, sizeof(buf), "Grid: %dx%d", LEVEL_ROWS[currentLevel + 1],
             LEVEL_COLS[currentLevel + 1]);
    DrawText(buf, cx - MeasureText(buf, 22) / 2, sh / 3 + 120, 22, DARKBLUE);
  } else {
    /* Beat all 3 levels */
    DrawText("YOU WIN!", cx - MeasureText("YOU WIN!", 50) / 2, sh / 3, 50,
             GOLD);
...
static void draw_gameover_screen(void) {
  ...
  if (gameResult == 1) {
    /* Won all levels */
    DrawText("VICTORY!", cx - MeasureText("VICTORY!", 50) / 2, sh / 4, 50,
             GOLD);
    ...
    /* Caught by bot */
    DrawText("GAME OVER", cx - MeasureText("GAME OVER", 50) / 2, sh / 4, 50,
             RED);
    snprintf(buf, sizeof(buf), "The bot caught you, %s!", playerName);
    DrawText(buf, cx - MeasureText(buf, 24) / 2, sh / 4 + 80, 24,
             (Color){255, 100, 100, 255});

    snprintf(buf, sizeof(buf), "Caught on Level %d, Turn %d", currentLevel + 1,
             turn_count);
...
void UpdateDrawFrame(void) {
  ...
  case GAMEPLAY: {
    ...
    /* Bot moves continuously, even if the player is idle */
    if (GetTime() - botMoveTimer >= botMoveInterval) {
      botMoveTimer = GetTime();
      move_bot();
...
int main(void) {
  ...
  /* Cleanup */
  if (audioLoaded) {
    if (IsSoundValid(winSound))
      UnloadSound(winSound);
    if (IsSoundValid(loseSound))
